...
void main() {
  ...
  test('redoCurrentLine undoes the previously completed line when the current line is untouched', () {
    final session = MarkingSession(_project(const [
      SubtitleLine(index: 0, text: 'a', startMs: 0, endMs: 100),
      SubtitleLine(index: 1, text: 'b'),
    ]));
  ...
  test('markStart and markEnd are no-ops once every line is fully marked', () {
    ...
    expect(session.lines[0], const SubtitleLine(index: 0, text: 'a', startMs: 0, endMs: 100));
    expect(session.currentIndex, isNull);
  });

  test('redoCurrentLine undoes the in-progress current line, not the previous completed one', () {
    final session = MarkingSession(_project(const [
      SubtitleLine(index: 0, text: 'a', startMs: 0, endMs: 100),
